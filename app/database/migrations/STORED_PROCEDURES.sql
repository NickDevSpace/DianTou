...
BEGIN
        ...
        UPDATE PROJECTS SET STATE = 'RAISE_SUCCESS' WHERE ID IN (SELECT PROJECT_ID FROM TMP_PROJECTS WHERE RSLT = 'RAISE_SUCCESS');
        ...
        UPDATE PROJECTS SET STATE = 'RAISE_FAILED' WHERE ID IN (SELECT PROJECT_ID FROM TMP_PROJECTS WHERE RSLT = 'RAISE_FAILED');
        INSERT INTO PROJECT_LIFE_EVENTS (PROJECT_ID, EVENT_TYPE, EVENT_DESC, EVENT_DATE, CREATED_AT, UPDATED_AT) 
...
CREATE EVENT IF NOT EXISTS E_PROJECT_STATE_SCAN ON SCHEDULE EVERY 300 SECOND ON COMPLETION PRESERVE DO CALL P_PROJECT_STATE_SCAN();//
